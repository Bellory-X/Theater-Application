...
//    @FXML private TableView<RankDTO> ranksTable;
    @FXML private TableView<CharactersDirectorDTO> charactersTable;
    @FXML private TableView<DirectorCharacterDTO> characterTable;
    @FXML private TableView<DirectorCategoryDTO> categoryTable;
    @FXML private TableView<DirectorDTO> actorTable;
    ...
    private final DirectorService actorService;
    private final DirectorCategoryService actorCategoryService;
    private final DirectorCharacterService actorCharacterService;
    private final CharactersDirectorService charactersActorService;
    ...
    public DirectorController(DirectorService actorService, DirectorCategoryService actorCategoryService,
                              DirectorCharacterService actorCharacterService,
                              CharactersDirectorService charactersActorService, FxWeaver fxWeaver) {
        ...
        try {
            switch (tableStatus) {
                ...
                case CHARACTERS -> {
                    ...
                    List<CharactersDirectorDTO> charactersList = charactersActorService.getAll().stream()
    ...
    private void addEvent() {
        try {
            switch (tableStatus) {
                case ACTOR -> {
                    actorService.add(DirectorDTO.builder().fullName(addField1.getText())
                            .experience(Integer.parseInt(addField2.getText()))
                            .gender(addField3.getText())
                            .birthday(Date.from(addField4.getValue().atStartOfDay().atZone(ZoneId.systemDefault()).toInstant()))
                            .countChild(Integer.parseInt(addField5.getText()))
                ...
                case CATEGORY -> {
                    actorCategoryService.add(DirectorCategoryDTO.builder().category(addField1.getText()).build());
                    categoryTable.setItems(FXCollections.observableList(actorCategoryService.getAll()));
                    categoryTable.refresh();
                }
                case CHARACTER -> {
                    actorCharacterService.add(DirectorCharacterDTO.builder().character(addField1.getText()).build());
                    characterTable.setItems(FXCollections.observableList(actorCharacterService.getAll()));
                    characterTable.refresh();
                }
                case CHARACTERS -> {
    ...
    private void searchEvent() {
        ...
        switch (queryStatus) {
            case QUERY0 -> {
                List<DirectorDTO> dtoList = actorService.getAll();
                result.setText(String.valueOf(dtoList.size()));
                actorTable.getItems().addAll(dtoList);
            }
            case QUERY1 -> {
                List<DirectorDTO> dtoList = actorService
