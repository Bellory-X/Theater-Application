...
public class DirectorController {
    ...
    @FXML private TableView<DirectorDTO> workerTable;
    ...
    private final DirectorService workerService;
    private final DirectorCategoryService categoryService;
    private final DirectorCharacterService characterService;
    private final CharactersDirectorService charactersService;
    private final FxWeaver fxWeaver;
    private final Map<WorkerQueryStatus, String> queryMap = new HashMap<>();

    public DirectorController(DirectorService workerService, DirectorCategoryService categoryService,
                              DirectorCharacterService characterService,
                            CharactersDirectorService charactersService, FxWeaver fxWeaver) {
    ...
    private void addEvent() {
        try {
            switch (tableStatus) {
                ...
                case CATEGORY -> {
                    categoryService.add(DirectorCategoryDTO.builder().category(addField1.getText()).build());
                    categoryTable.setItems(FXCollections.observableList(categoryService.getAll()));
                }
                case CHARACTER -> {
                    characterService.add(DirectorCharacterDTO.builder().character(addField1.getText()).build());
                    characterTable.setItems(FXCollections.observableList(characterService.getAll()));
                }
                case CHARACTERS -> {
                    if (workerTable.getSelectionModel().getSelectedItem() == null ||
    ...
    private void searchEvent() {
        switch (queryStatus) {
            case QUERY0 -> {
                List<DirectorDTO> dtoList = workerService.getAll();
                result.setText(String.valueOf(dtoList.size()));
                workerTable.setItems(FXCollections.observableList(dtoList));
            }
            case QUERY1 -> {
                try {
                    List<DirectorDTO> dtoList = workerService.findActorQuery1(
